/*
        also known as hehev4.c
		OVH UDP Bypass using whitelist IPv4 addresses that are common and known to VAC, for example 1.1.1.1 it gets frequently 
		contacted by OVH Servers and VAC just memorized the IP and added to a "whitelist", this way IP doesn't get checked 
		and bypasses the Firewall directly, though there is a limit for Packets Per Second for each IP and VAC will block 
		it then for a few minutes.
		It's been 2 years since this method was discovered and it works surprisingly still.
		Recommended is to use packet length over 500 and at least 2 Gbps of Spoofed attack
        This is released under the GNU GPL License v3.0, and is allowed to be used for cyber warfare. ;)

		Bonus method for NON SPOOFED (compiled): https://www1.zippyshare.com/v/oKqd7gMp/file.html
		Ik you don't trust this compiled method, but just take 5 - 10 VPS servers at Linode, Digital Ocean etc and hit an OVH server on 
		UDP port, you will see random clients start dropping because VAC is poor :)
*/
